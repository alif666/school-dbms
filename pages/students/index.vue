<template>
    <v-app>
      <v-container>
        ...
        <v-card flat>
          ...
          <v-card-title class="d-flex align-center pe-2">
            <v-btn class="mb-2" color="primary" dark @click="navigateTo('/students/form')">
            ...
            <v-row>
              <v-col v-for="(item, index) in items" :key="index" cols="12" sm="6" md="4" lg="3">
                <v-card class="my-4" elevation="2">
                  <!-- Staff Image -->
                  <v-img :src="item.image_url" height="150" cover></v-img>
  
                  <!-- Staff Info -->
                  <v-card-title>{{ item.name }} [{{ item.professional_qualification }}]</v-card-title>
                  <v-card-subtitle>Academic {{ item.academic_qualification }}</v-card-subtitle><v-spacer />
                  <v-card-subtitle>ID: {{ item.id }}</v-card-subtitle>
                  <v-card-subtitle>Contact: {{ item.contact_mobile }}</v-card-subtitle>
  
  
                  <v-card-text>Classes: {{ item.classes_taught }}</v-card-text>
                  <v-card-text>Subject: {{ item.subjects_taught }}</v-card-text>
          ...
          <v-dialog v-model="insertDialog" max-width="600px">
            <v-card>
              ...
              <v-card-text>
                ...
                <v-autocomplete v-model="newStaff.subjects_taught" :items="subjectLookups" label="Subjects Taught"
                ...
                <v-card-text>
                  <dropzone :accepted-files="['image']" dropzone-class-name="dropzone-box" :multiple="false"
  ...
  import { supabase, supabaseStorageUrl } from '~/supabase'; // Adjust path based on your project structure
